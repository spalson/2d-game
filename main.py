    ...
    def physic_tick(self, beams):
        ...
        for beam in beams:
            if beam.hitbox.colliderect(self.hitbox):  # cofanie obiektu do miejsca z poprzedniej klatki
                if self.x_cord + self.width >= beam.x_cord + 1 > self.previous_x + self.width:  # kolizja z prawej
                    self.x_cord = self.previous_x
                    self.hor_velocity = 0
                if self.x_cord <= beam.x_cord - 1 < self.previous_x:  # kolizja z lewej strony
                    self.x_cord = self.previous_x
                    self.hor_velocity = 0
                if self.y_cord + self.height >= beam.y_cord + 1 > self.previous_y:  # kolizja z góry
                    self.y_cord = self.previous_y
                    self.ver_velocity = 0
                if self.y_cord <= beam.x_cord + self.width - 1 < self.previous_y:  # kolizja z dołu
                    self.y_cord = self.previous_y
                    self.ver_velocity = 0
...
class Player(Physic):
    def __init__(self):
        self.image = pygame.image.load("buka.png")  # wczytuje grafikę
